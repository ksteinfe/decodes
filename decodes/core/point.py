...
class Point(dc.Vec,dc.HasBasis):
  ...
  def _distance2(self, other):
    if self.basis is not other.basis : raise dc.BasisError("Cannot measure '_distance2' between points with different bases.  Use 'distance2' instead")
    return dc.Vec(self,other).length2
  def _distance(self, other): 
    if self.basis is not other.basis : raise dc.BasisError("Cannot measure '_distance' between points with different bases.  Use 'distance' instead")
    return dc.Vec(self,other).length
  def distance2(self,other): return dc.Vec(self.basis_applied(),other.basis_applied()).length2
  def distance(self,other): return dc.Vec(self.basis_applied(),other.basis_applied()).length
  
  @staticmethod
  def interpolate(p0,p1,t=0.5): 
    if p0.basis is p1.basis : 
      v = dc.Vec.interpolate(p0,p1,t)
      return Point(v.x,v.y,v.z,p0.basis)
    else : 
      v = dc.Vec.interpolate(p0.basis_applied(),p1.basis_applied(),t)
      return Point(v.x,v.y,v.z)

  @staticmethod
  def _centroid(points): 
    if all( points[0].basis is p.basis for p in points ) : 
      cent = dc.Vec.average([p.basis_stripped() for p in points])
      return Point(cent).set_basis(points[0].basis)
    raise dc.BasisError("Cannot calculate '_centroid' for points of mixed bases.  Use 'centroid' instead")
    
  @staticmethod
  def centroid(points): 
    return Point( dc.Vec.average([p.basis_applied() for p in points]) )
    
  '''Returns a new point projected onto a destination vector'''
  def projected(self, other): return Point( dc.Vec(self.x,self.y,self.z).projected(other) )
