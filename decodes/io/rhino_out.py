...
from . import outie
if VERBOSE_FS: print "rhinoOut loaded"
...
class RhinoOut(outie.Outie):
  ...
  def __init__(self, layername):
    ...
    layerindex = RhinoOut.makelayer(layername)
  ...
  def _drawVec(self, vec, obj_attr):
    origin = Vec(0,0,0)
    guid = scriptcontext.doc.Objects.AddLine(RhinoOut.to_rgpt(origin),RhinoOut.to_rgpt(origin+vec),obj_attr)
    return guid!=System.Guid.Empty

  def _drawPoint(self, pt, obj_attr):
    pt = pt.basis_applied()
    guid = scriptcontext.doc.Objects.AddPoint(RhinoOut.to_rgpt(pt),obj_attr)
    return guid!=System.Guid.Empty
  ...
  def _drawLinearEntity(self, ln, obj_attr):
    ...
    if isinstance(ln, Segment) : 
      guid = sDocObj.AddLine(RhinoOut.to_rgpt(ln.spt),RhinoOut.to_rgpt(ln.ept),obj_attr)
      return guid!=System.Guid.Empty
    if isinstance(ln, Ray) : 
      p = sDocObj.AddPoint(RhinoOut.to_rgpt(ln.spt),obj_attr)
      l = sDocObj.AddLine(RhinoOut.to_rgpt(ln.spt),RhinoOut.to_rgpt(ln.ept),obj_attr)
      scriptcontext.doc.Groups.Add([p,l])
    if isinstance(ln, Line) : 
      p = sDocObj.AddPoint(RhinoOut.to_rgpt(ln.spt),obj_attr)
      l = sDocObj.AddLine(RhinoOut.to_rgpt(ln.spt-ln.vec/2),RhinoOut.to_rgpt(ln.spt+ln.vec/2),obj_attr)
      scriptcontext.doc.Groups.Add([p,l])
  ...
  def _drawCS(self, cs, obj_attr):
    ...
    rh_circ = Rhino.Geometry.Circle(Rhino.Geometry.Plane(RhinoOut.to_rgpt(cs.origin),RhinoOut.to_rgvec(cs.zAxis)), self.iconscale*0.5)
    c = sDocObj.AddCircle(rh_circ,obj_attr)
    
    obj_attr.ObjectColor = System.Drawing.Color.FromArgb(255,0,0)  
    x = sDocObj.AddLine(RhinoOut.to_rgpt(cs.origin), RhinoOut.to_rgpt(cs.origin+(cs.xAxis*self.iconscale)),obj_attr)
    obj_attr.ObjectColor = System.Drawing.Color.FromArgb(0,255,0)
    y = sDocObj.AddLine(RhinoOut.to_rgpt(cs.origin), RhinoOut.to_rgpt(cs.origin+(cs.yAxis*self.iconscale)),obj_attr)
    obj_attr.ObjectColor = System.Drawing.Color.FromArgb(0,0,255)
    z = sDocObj.AddLine(RhinoOut.to_rgpt(cs.origin), RhinoOut.to_rgpt(cs.origin+(cs.zAxis*0.5*self.iconscale)),obj_attr)
    obj_attr.ObjectColor = System.Drawing.Color.FromArgb(255,255,255)
    o = sDocObj.AddPoint(RhinoOut.to_rgpt(cs.origin),obj_attr)
  ...
  def _drawCylCS(self, cs, obj_attr):
    ...
    obj_attr.ObjectColor = System.Drawing.Color.FromArgb(255,0,0)  
    x = sDocObj.AddLine(RhinoOut.to_rgpt(cs.origin), RhinoOut.to_rgpt(cs.origin+(Vec(1,0,0)*self.iconscale)),obj_attr)
    obj_attr.ObjectColor = System.Drawing.Color.FromArgb(0,255,0)
    rh_circ = Rhino.Geometry.Circle(Rhino.Geometry.Plane(RhinoOut.to_rgpt(cs.origin),RhinoOut.to_rgvec(Vec(0,0,1))), self.iconscale*0.5)
    y = sDocObj.AddCircle(rh_circ,obj_attr)
    rh_circ = Rhino.Geometry.Circle(Rhino.Geometry.Plane(RhinoOut.to_rgpt(cs.origin+Vec(0,0,self.iconscale*0.5)),RhinoOut.to_rgvec(Vec(0,0,1))), self.iconscale*0.5)
    yy = sDocObj.AddCircle(rh_circ,obj_attr)
    obj_attr.ObjectColor = System.Drawing.Color.FromArgb(0,0,255)
    z = sDocObj.AddLine(RhinoOut.to_rgpt(cs.origin), RhinoOut.to_rgpt(cs.origin+(Vec(0,0,1)*0.5*self.iconscale)),obj_attr)
    obj_attr.ObjectColor = System.Drawing.Color.FromArgb(255,255,255)
    o = sDocObj.AddPoint(RhinoOut.to_rgpt(cs.origin),obj_attr)
  ...
  def to_rgpolyline(pgon):
    verts = [RhinoOut.to_rgpt(pt) for pt in pgon.verts]
    verts.append(verts[0])
    return Rhino.Geometry.Polyline(verts)

  @staticmethod
  def to_rgplane(cs):
    return Rhino.Geometry.Plane(RhinoOut.to_rgpt(cs.origin),RhinoOut.to_rgvec(cs.xAxis),RhinoOut.to_rgvec(cs.yAxis))
