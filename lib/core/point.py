...
from fieldpack import Vec
import math, random
if fp.VERBOSE_FS: print "point.py loaded"
...
class Point(Vec,fp.HasBasis):
  ...
  def __mul__(self, other):
    if isinstance(other, fp.Xform) :
      ...
      return fp.Xform.scale(other) * self
  ...
  def _distance2(self, other):
    if self.basis is not other.basis : raise fp.BasisError("Cannot measure '_distance2' between points with different bases.  Use 'distance2' instead")
    return Vec(self,other).length2
  def _distance(self, other): 
    if self.basis is not other.basis : raise fp.BasisError("Cannot measure '_distance' between points with different bases.  Use 'distance' instead")
  ...
  def _centroid(points): 
    ...
    raise fp.BasisError("Cannot calculate '_centroid' for points of mixed bases.  Use 'centroid' instead")
