...
class DzTile(object):
  ...
  def _PositionByChildAndParentCS(self,csChild,csParent,flip):
    '''
  ...
  def _PositionByParentCS(self,childTile,csParent,rlvl,flip):
    '''
    Finds the proper XForm for a child tile given:
    childTile: a child tile (using default construction, ex: DxTileA()
    csParent : a target CS positioned on parent's base points at rlvl = 0
    rlvl: the rlvl of the parent
    flip: flip this child tile?
    '''
    cbPts = childTile._scaledBasePts(rlvl+1) # child's base points (inflated to rlvl+1)
    csChild = CS()
    xf = self._PositionByChildAndParentCS(csChild,csParent,flip)
    childTile.xf = xf
    childTile.flip = flip
    childTile.rlvl = rlvl + 1
    return childTile
...
class DzTileB(DzTile):
  ...
  def inflate(self):
    ...
    xf = self._PositionByParentCS(DzTileB(),parentCS,self.rlvl,not self.flip)  # opposite handdeness as parent
    tileB0 = DzTileB(xf=xf,rlvl=self.rlvl+1,flip=not self.flip) # opposite handdeness as parent
    
    #DzTileK by orienting child origin to defined CS on parent
    parentCS = CS(pbPts[3], pbPts[0]-pbPts[3], pbPts[2]-pbPts[3])
    xf = self._PositionByParentCS(DzTileK(),parentCS,self.rlvl,self.flip)# same handedness as parent
    tileK0 = DzTileK(xf=xf,rlvl=self.rlvl+1,flip=self.flip) # same handedness as parent
    ...
    tileK1 = DzTileK(xf=xf_rot*xf,rlvl=self.rlvl+1,flip=self.flip)# same handedness as parent
    ...
    xf = self._PositionByChildAndParentCS(csChild,csParent,flip=not self.flip) # opposite handedness as parent
    tileK2 = DzTileK(xf=xf,rlvl=self.rlvl+1,flip=not self.flip) # opposite handedness as parent
    #DzTileK by rotating the previous tile
    xf_rot = Xform.rotation(center=tileK2._tilePts()[0], angle=math.pi, axis=tileK2._tilePts()[3]-tileK2._tilePts()[0])
    tileK3 = DzTileK(xf=xf_rot*xf,rlvl=self.rlvl+1,flip=not self.flip)  # opposite handedness as parent
    ...
    xf = self._PositionByChildAndParentCS(csChild,csParent,flip=self.flip) # same handedness as parent
    tileB1 = DzTileB(xf=xf,rlvl=self.rlvl+1,flip= self.flip) # same handedness as parent
    
    #DzTileC by orienting child origin to defined CS on parent
    parentCS = CS(pbPts[3],tileK0._tilePts()[2]-pbPts[3], pbPts[2]-pbPts[3])
    xf = self._PositionByParentCS(DzTileC(),parentCS,self.rlvl,self.flip)  # same handedness as parent
    tileC0 = DzTileC(xf=xf,rlvl=self.rlvl+1,flip=self.flip)  # same handedness as parent
...
class DzTileK(DzTile):
  ...
  def inflate(self):
    ...
    flip = self.flip # same handedness as parent
    xf = self._PositionByChildAndParentCS(csChild,csParent,flip)
    iTiles.append(DzTileK(xf=xf,rlvl=self.rlvl+1))
    
    #DzTileB by orienting child origin to defined CS on parent
    parentCS = CS(pbPts[2],pbPts[2]*-1,Vec(1,0,0))
    flip = self.flip # same handedness as parent
    xf = self._PositionByParentCS(DzTileB(),parentCS,self.rlvl,flip)
    iTiles.append(DzTileB(xf=xf,rlvl=self.rlvl+1,flip=flip))
